...
func TestWebHttpInfoFaviconURLSupportsRelIcon(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = w.Write([]byte(`<html><head><link rel="icon" href="/favicon-alt.ico"></head><title>ok</title></html>`))
	})
	mux.HandleFunc("/favicon-alt.ico", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "image/x-icon")
		_, _ = w.Write([]byte("ico-alt"))
	})
	server := httptest.NewServer(mux)
	defer server.Close()
	...
	if len(info.Favicon) == 0 {
		t.Fatal("expected favicon body from rel=icon")
	}
}
